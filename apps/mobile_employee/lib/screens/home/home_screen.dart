    ...
    return Scaffold(
      ...
      bottomNavigationBar: Container(
        ...
        child: BottomNavigationBar(
          ...
          onTap: (i) {
            setState(() => _tab = i);
            if (i == 3 && isManager) _loadPendingCount();
          },
          elevation: 0,
          ...
          items: [
            ...
            BottomNavigationBarItem(
              icon: _BadgeIcon(
                ...
                count: isManager ? _pendingCount : 0,
              ),
              activeIcon: Icon(
                isManager
...
class _CommunityPlaceholder extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: const Center(
      child: Text('قريباً...', style: TextStyle(color: kTextSub, fontSize: 16)),
    ),
  );
}
...
class _HomeTabState extends State<_HomeTab> {
  ...
  Future<void> _load() async {
    try {
      ...
      final leaveData = results[1].data is List
          ? results[1].data
          : (results[1].data['data'] ?? []);
      final appraisalData = results[2].data is List
          ? results[2].data
          : (results[2].data['data'] ?? []);
      final feedData = results[3].data is List
          ? results[3].data
          : (results[3].data['data'] ?? []);
      // عداد الطلبات المعلّقة (إجازات + طلبات أخرى)
      final pendingLeave = (leaveData as List)
          .where(
...
class _DashboardWidgets extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final approved = announcements
        .where((r) => r['postType'] != null)
        .length; // عدد الأخبار المقروءة
        ...
        children: [
          ...
          Expanded(
            child: _Widget(
              ...
              bg: kPurpleLight,
              label: 'آخر تقييم',
              value: lastScore != null
                  ? '${double.tryParse(lastScore.toString())?.round()}%'
                  : '—',
...
class _TalentLogo extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) =>
      CustomPaint(size: Size(size, size), painter: _MountainPainter());
}
...
class _StoryStrip extends StatelessWidget {
  ...
  const _StoryStrip({required this.items});
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (_, index) {
          if (index == 0) {
            return _StoryCard(
              ...
              gradient: const [Color(0xFF1877F2), Color(0xFF3B82F6)],
            );
          }

          final item = items[index - 1];
          final title =
              (item['author']?['fullName'] ?? item['title'] ?? 'أخبار')
                  .toString();
          return _StoryCard(
            title: title,
        ...
        itemCount: (items.length.clamp(0, 5)) + 1,
      ),
    );
  }
}
...
class _ComposerCard extends StatelessWidget {
  ...
  const _ComposerCard({required this.user});
...
class _NewsCardState extends State<_NewsCard> {
  ...
  Future<void> _toggleLike() async {
    ...
    try {
      ...
      await ApiClient().dio.post(
        '/communication/posts/${widget.r['id']}/react',
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        if (isPinned) ...[
                          const SizedBox(width: 4),
                          const Icon(Icons.push_pin, size: 12, color: kWarning),
                        ],
                      ],
                    ),
                    Text(
  ...
  String _fmt(dynamic d) {
    try {
      return (d as String).substring(0, 10);
    } catch (_) {
      return '';
    }
  }
  ...
  Future<void> _load() async {
    try {
      final res = await ApiClient().dio.get(
        '/communication/posts/${widget.postId}/comments',
      );
      ...
      children: [
        ...
        Expanded(
          child: Column(
            ...
            children: [
              Text(
                'أهلاً ${user.fullName.split(' ').first}!',
                style: const TextStyle(
                  fontSize: 14,
                  fontWeight: FontWeight.w700,
                  color: kText,
